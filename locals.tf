locals {
  ...
  db_url                   = "jdbc:mysql://${local.db_host}:3306/${local.db_name}?useUnicode=true&characterEncoding=UTF-8&user=${local.db_username}&password=${local.db_password}&useSSL=true&allowPublicKeyRetrieval=true"
  efs_id                   = var.efs_id
