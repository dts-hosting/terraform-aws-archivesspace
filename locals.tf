locals {
  ...
  db_ssl                   = var.db_use_ssl ? "useSSL=true&requireSSL=true&verifyServerCertificate=true&trustCertificateKeyStoreUrl=file:///certs/global-bundle.pem" : "useSSL=false"
  db_url                   = "jdbc:mysql://${local.db_host}:3306/${local.db_name}?useUnicode=true&characterEncoding=UTF-8&user=${local.db_username}&password=${local.db_password}&${local.db_ssl}&allowPublicKeyRetrieval=true"
  efs_id                   = var.efs_id
